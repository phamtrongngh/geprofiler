...
def _get_active_profiler():
    """
    Allows the code inserted into the cell to access the geprofiler Profiler
...
@magics_class
class GeprofilerMagic(Magics):
    def __init__(self, shell):
        super().__init__(shell)
        # This will leak _get_active_profiler into the users space until we can magle it
        self.pre = parse(
            "\nfrom geprofiler.magic.magic import _get_active_profiler; _get_active_profiler().start()\n"
        )
    ...
    @argument(
        ...
        type=float,
        default=0.001,
        help="The minimum time, in seconds, between each stack sample. See: https://geprofiler.readthedocs.io/en/latest/reference.html#geprofiler.Profiler.interval",
    )
    @argument(
        "--async_mode",
        default="enabled",
        help="Configures how this Profiler tracks time in a program that uses async/await. See: https://geprofiler.readthedocs.io/en/latest/reference.html#geprofiler.Profiler.async_mode",
    )
    @argument(
        "--height",
        "-h",
    ...
    @line_cell_magic
    def geprofiler(self, line, cell=None):
        """
        Run a cell with the geprofiler statistical profiler.
        ...
        args = parse_argstring(self.geprofiler, line)
        ip = get_ipython()
