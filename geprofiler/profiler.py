...
from geprofiler import renderers
from geprofiler.frame import AWAIT_FRAME_IDENTIFIER, OUT_OF_CONTEXT_FRAME_IDENTIFIER
from geprofiler.session import Session
from geprofiler.stack_sampler import AsyncState, StackSampler, build_call_stack, get_stack_sampler
from geprofiler.typing import LiteralStr
from geprofiler.util import file_supports_color, file_supports_unicode
...
class Profiler:
    """
    The profiler - this is the main way to use geprofiler.
    """
    ...
    def start(self, caller_frame: types.FrameType | None = None):
        ...
        :param caller_frame: Set this to override the default behaviour of treating the caller of
            ...
            writing a library that wraps geprofiler.
        """
    ...
    def _sampler_saw_call_stack(
        ...
        if not self._active_session:
            raise RuntimeError(
                "Received a call stack without an active session. Please file an issue on geprofiler Github describing how you made this happen!"
            )
