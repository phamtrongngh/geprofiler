  ...
  <div>
    <div class="frame">
      <div
        ...
        v-if="isVisible"
        :class="{
          'application-code': frame.isApplicationCode,
      ...
      >
        <div
          ...
          :style="{
            visibility: frame.children.length > 0 ? 'visible' : 'hidden',
          }"
        >
          <svg width="6" height="10">
            <path
              d="M.937-.016L5.793 4.84.937 9.696z"
              :fill="timeStyle.color"
              fill-rule="evenodd"
              fill-opacity=".582"
            />
          </svg>
      ...
      <div
        v-if="frame.group && frame.group.rootFrame == frame && childrenVisible"
        class="group-header"
        :style="{ paddingLeft: `${indent * 35}px` }"
...
export default {
  ...
  computed: {
    ...
    codePosition() {
      return `${this.frame.filePathShort}:${this.frame.lineNo
        .toString()
        .padEnd(4, " ")}`;
    },
    formattedTime() {
